...
export function fetchCrowdSaleContract(address: Address, tokenAmount: BigInt): CrowdSaleContract {
  ...
  if (contract == null) {
    ...
    contract.initialSupply = tokensAvailable.reverted ? null : tokensAvailable.value.plus(tokenAmount);
    contract.tokensAvailable = tokensAvailable.reverted ? null : tokensAvailable.value.plus(tokenAmount);
